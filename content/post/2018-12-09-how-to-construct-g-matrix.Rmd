...
现在计算Ind1和Ind2个体间的亲缘关系，= $G_{12}/\sqrt{G_{11}G_{22}}$ = `r G[1,2]/(G[1,1]*G[2,2])`。

由于对角线元素一般都在1左右，因此j与k的亲缘（relationship）系数也就基本约等于G矩阵中相对应的值。

因此这里可以G阵看做传统A阵，那么j与k的共亲系数（coancestry）也就是G阵中相对应的亲缘系数的一半。
